...
try {
    $stmt = $pdo->prepare('SELECT * FROM publisherPhp ORDER BY editora');
    $stmt->execute();

    $fp = fopen('../csvFile/publisher.csv', 'w');

    $titleColumns = array('editora', 'livro');
